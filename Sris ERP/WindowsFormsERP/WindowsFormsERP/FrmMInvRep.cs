...
{
    public partial class FrmMInvRep : Form
    {
        int count = 0;
        ...
        public void EPDFReport(DataGridView dgw, string filename)
        {
            ...
            {
                float[] widths = new float[] { 0f, 65f, 80f, 0f, 80f, 0f, 0f, 0f, 240f, 0f, 0f, 50f, 75f, 85f, 0f, 0f, 0f, 0f, 0f, 0f, 0f, 120f, 0f, 0f, 0f, 0f, 0f, 0f };
                pdftable.SetWidths(widths);
            }
        ...
        private void FrmMInvRep_Load(object sender, EventArgs e)
        {
            count = 0;
            this.label1.Text = this.Text;
            if (this.Text == "Manufacture Invoice Report (Total by Item)")
            {
                this.label4.Text = "Item";
            }
            else if (this.Text == "Manufacture Invoice Report (Total by Customer)")
            {
        ...
        private void TxtSer_TextChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                if (this.label4.Text == "Item")
                {
                   // cmd.CommandText = "select * from ManufactureCompletedDetails where Description Like '%" + TxtSer.Text + "%'";
                    cmd.CommandText = "select Description, sum(BillQuantity) as Qty, sum(ItemTotal) as Amount from ManufactureCompletedDetails where Description Like '%" + TxtSer.Text + "%' GROUP BY Description";
                }
                else if (this.label4.Text == "Customer Name")
                {
                    //cmd.CommandText = "select * from ManufactureCompletedDetails where CustomerName Like '%" + TxtSer.Text + "%'";
                    cmd.CommandText = "select CustomerName, sum(ItemTotal) as Amount from ManufactureCompletedDetails where CustomerName Like '%" + TxtSer.Text + "%' GROUP BY CustomerName";
                }
                else
                {
                    cmd.CommandText = "select * from ManufactureCompletedDetails where BillNumber Like '%" + TxtSer.Text + "%' or BillCategory Like '%" + TxtSer.Text + "%' or CustomerName Like '%" + TxtSer.Text + "%' or Description Like '%" + TxtSer.Text + "%' or LocationID Like '%" + TxtSer.Text + "%' or ManualReferenceNo Like '%" + TxtSer.Text + "%' or SalesCategoryKeyword Like '%" + TxtSer.Text + "%' or StockNumber Like '%" + TxtSer.Text
                        + "%' or MarketingStaffName Like '%" + TxtSer.Text + "%' or CustomerAddress Like '%" + TxtSer.Text + "%' or PONumber Like '%" + TxtSer.Text + "%' or PaymentTerms Like '%" + TxtSer.Text + "%' or JobOrderNo Like '%" + TxtSer.Text + "%' or CreatedBy Like '%" + TxtSer.Text + "%'";
                }
                cmd.ExecuteNonQuery();
            ...
            {
                MessageBox.Show("ERROR CODE : SER-CNG-103-MINV-REP" + "\n" + "\n" + "[Please Note this Error Code and Take a Photo in this More Details." + "\n" + "\n" + "Inform the Error Code and Send this Error Details (Mail or WhatsUP) to Development Team (SRIS)!]" + "\n" + "\n" + "\n" + "MORE DETAILS :- " + "\n" + "\n" + x, "MESSAGE BOX", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
