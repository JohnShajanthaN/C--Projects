...
{
    public partial class FrmAccPayable : Form
    {
        int dgvRows = 0;
        double pp1;
        ...
        public void EPDFReport(DataGridView dgw, string filename)
        {
            ...
            float[] widths = new float[] { 0f, 75f, 100f, 0f, 210f, 65f, 220f, 70f, 70f, 60f, 85f, 120f };
            ...
            foreach (DataGridViewRow row in dgw.Rows)//Add Data Row  
            {
                foreach (DataGridViewCell cell in row.Cells)
                {
                    ...
                    else if (cell.ColumnIndex.ToString() == "8" || cell.ColumnIndex.ToString() == "10" || cell.ColumnIndex.ToString() == "11")
                    {
            ...
            if (savefiledialoge.ShowDialog() == DialogResult.OK)
            {
                using (FileStream stream = new FileStream(savefiledialoge.FileName, FileMode.Create))
                {
                    ...
                    pdfdoc.Open();
                    Paragraph para = new Paragraph("Account Payable Report");
        ...
        private void FrmAccPayable_Load(object sender, EventArgs e)
        {
        ...
        {
            EPDFReport(dataGridView1, "Account Payable Report - " + DateTime.Now.ToString("dd.MM.yyyy"));
            OpenFileDialog dlg = new OpenFileDialog();
            dlg.Filter = "pdf files (*.pdf) |*.pdf;";
            dlg.FileName = ("Account Payable Report - " + DateTime.Now.ToString("dd.MM.yyyy") + ".pdf");
            dlg.ShowDialog();
        ...
        private void TxtSer_TextChanged(object sender, EventArgs e)
        {
            try
            {
               ...
               // cmd.CommandText = "select * from IOU where IOUNo Like '%" + TxtSer.Text + "%' or IOUName Like '%" + TxtSer.Text + "%' or Reason Like '%" + TxtSer.Text + "%' or Item Like '%" + TxtSer.Text + "%'";
                cmd.CommandText = "select PurchaseGRN.PONO, PurchaseGRN.GRNNo, PurchaseGRN.GRNDate, PurchaseGRN.SupplierCode As SCode, PurchaseGRN.Supplier, PurchaseGRN.StockNumber As StockNo,  PurchaseGRN.Description, PurchaseGRN.Weight, PurchaseGRN.RvdQty, PurchaseGRN.Unit, Purchase.CostPrice, (Purchase.CostPrice*PurchaseGRN.RvdQty) As Payable from PurchaseGRN INNER JOIN Purchase ON PurchaseGRN.PONO=Purchase.PONO where PurchaseGRN.PONO Like '%" + TxtSer.Text + "%' or PurchaseGRN.GRNNo Like '%" + TxtSer.Text + "%' or PurchaseGRN.Supplier Like '%" + TxtSer.Text + "%' or PurchaseGRN.Description Like '%" + TxtSer.Text + "%';";
                                                                                                                                                                 //   where PurchaseGRN.PONO Like '%" + TxtSer.Text + "%' or PurchaseGRN.GRNNo Like '%" + TxtSer.Text + "%' or PurchaseGRN.Supplier Like '%" + TxtSer.Text + "%' or PurchaseGRN.Description Like '%" + TxtSer.Text + "%'";
                cmd.ExecuteNonQuery();
                ...
                {
                    pp1 = pp1 + Convert.ToDouble(dataGridView1.Rows[k].Cells[11].Value);
                }
                lblTot.Text = "Rs. " + pp1.ToString("#,###.00");
            }
            catch (Exception x)
            {
