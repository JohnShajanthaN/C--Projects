...
{
    public partial class FrmChkPaySupRep : Form
    {
        int dgvRows = 0;
        string Lst1 = "";
        double pp1, qq1;
        ...
        private void DGViewSize()
        {
           ...
           // dataGridView1.Columns[8].Visible = false;
            ...
            con.Open();
            OleDbCommand cmd1 = new OleDbCommand("select * from VoucherChequePayment where Sup='" + Lst1 + "' AND (Bank Like '%" + TxtSer.Text + "%' OR ChqNo Like '%" + TxtSer.Text + "%' OR AccNo Like '%" + TxtSer.Text + "%' OR PVNo Like '%" + TxtSer.Text + "%') ", con);
           // OleDbCommand cmd1 = new OleDbCommand("select * from VoucherChequePayment where Bank='" + Lst1 + "' AND (Sup Like '%" + TxtSer.Text + "%' OR ChqNo Like '%" + TxtSer.Text + "%' OR AccNo Like '%" + TxtSer.Text + "%' OR PVNo Like '%" + TxtSer.Text + "%') ", con);
            ...
            while (rdr.Read())
            {
                ...
                dataGridView1.Rows[dgvRows - 1].Cells[6].Value = rdr["AccNo"].ToString();
                dataGridView1.Rows[dgvRows - 1].Cells[7].Value = Convert.ToDouble(rdr["Amount"].ToString());
               ...
               // pp1 = pp1 + Convert.ToDouble(dataGridView1.Rows[dgvRows - 1].Cells[7].Value);
                qq1 = qq1 + Convert.ToDouble(dataGridView1.Rows[dgvRows - 1].Cells[7].Value);
            ...
            dataGridView1.Rows[dgvRows].Cells[7].Value = Convert.ToDouble(qq1.ToString());
        ...
        public void EPDFReport(DataGridView dgw, string filename)
        {
            ...
            float[] widths = new float[] { 50f, 100f, 110f, 110f, 140f, 190f, 150f, 120f };
            ...
            foreach (DataGridViewRow row in dgw.Rows)//Add Data Row  
            {
                foreach (DataGridViewCell cell in row.Cells)
                {
                    ...
                    if (cell.ColumnIndex.ToString() == "2" || cell.ColumnIndex.ToString() == "3")
                    {
                        cell1.HorizontalAlignment = Element.ALIGN_CENTER;
                    }
                    else if (cell.ColumnIndex.ToString() == "0" || cell.ColumnIndex.ToString() == "7")
                    {
            ...
            if (savefiledialoge.ShowDialog() == DialogResult.OK)
            {
                using (FileStream stream = new FileStream(savefiledialoge.FileName, FileMode.Create))
                {
                    ...
                    Paragraph para0 = new Paragraph("Cheque Payment - " + DateTime.Now.ToString("dd.MM.yyyy"));
        ...
        public FrmChkPaySupRep()
        {
            InitializeComponent();
        }

        private void FrmChkPaySupRep_Load(object sender, EventArgs e)
        {
            Lst1 = FrmChqPaySupList.Lst;
            this.label1.Text = "Cheque Payment - " + this.Text;
            Refresh();
            this.Refresh();
            Refresh();
            this.Refresh();
        ...
        private void FrmChkPaySupRep_FormClosing(object sender, FormClosingEventArgs e)
        {
            FrmChqPaySupList FChqPaySupList = new FrmChqPaySupList();
            FChqPaySupList.Visible = true;
        }
