...
{
    public partial class FrmTaxInvRep : Form
    {
        int count = 0;
        ...
        private void FrmTaxInvRep_Load(object sender, EventArgs e)
        {
            count = 0;
            this.label1.Text = this.Text;
            if (this.Text == "Tax Invoice Report (Total by Item)")
            {
                this.label4.Text = "Item";
            }
            else if (this.Text == "Tax Invoice Report (Total by Customer)")
            {
        ...
        private void TxtSer_TextChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                if (this.label4.Text == "Item")
                {
                   // cmd.CommandText = "select * from TaxCompletedDetails where Description Like '%" + TxtSer.Text + "%'";
                    cmd.CommandText = "select Description, sum(BillQuantity) as Qty, sum(ItemTotal) as Amount from TaxCompletedDetails  where Description Like '%" + TxtSer.Text + "%' GROUP BY Description";
                }
                else if (this.label4.Text == "Customer Name")
                {
                    //cmd.CommandText = "select * from TaxCompletedDetails where CustomerName Like '%" + TxtSer.Text + "%'";
                    cmd.CommandText = "select CustomerName, sum(ItemTotal) as Amount from TaxCompletedDetails where CustomerName Like '%" + TxtSer.Text + "%' GROUP BY CustomerName";
                }
                else
                {
                    cmd.CommandText = "select * from TaxCompletedDetails where BillNumber Like '%" + TxtSer.Text + "%' or BillCategory Like '%" + TxtSer.Text + "%' or CustomerName Like '%" + TxtSer.Text + "%' or Description Like '%" + TxtSer.Text + "%' or LocationID Like '%" + TxtSer.Text + "%' or ManualReferenceNo Like '%" + TxtSer.Text + "%' or SalesCategoryKeyword Like '%" + TxtSer.Text + "%' or StockNumber Like '%" + TxtSer.Text
            ...
            {
                MessageBox.Show("ERROR CODE : SER-CNG-104-TINV-REP" + "\n" + "\n" + "[Please Note this Error Code and Take a Photo in this More Details." + "\n" + "\n" + "Inform the Error Code and Send this Error Details (Mail or WhatsUP) to Development Team (SRIS)!]" + "\n" + "\n" + "\n" + "MORE DETAILS :- " + "\n" + "\n" + x, "MESSAGE BOX", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
