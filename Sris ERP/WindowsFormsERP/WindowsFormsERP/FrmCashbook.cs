      ...
      // int count = 0;
       // int count1 = 0;
        ...
        private void DGViewSize()
        {
            ...
            con.Open();
            OleDbCommand cmd1 = new OleDbCommand("select * from Journal where Debit > 0 AND  Particular='" + "Cash" + "' ", con);
            cmd1.Connection = con;
            ...
            con.Open();
            OleDbCommand cmd2 = new OleDbCommand("select * from Journal where Credit > 0 AND  Particular='" + "Cash" + "' ", con);
            cmd2.Connection = con;
            ...
            if (LL1 > RR1)
            {
                ...
                dataGridView1.Rows[dgvRows4 + 2].Cells[1].Style.Font = label7.Font;
                dataGridView1.Rows[dgvRows4 + 2].Cells[2].Style.Font = label7.Font;
                CreditDebit = "Debit";
            }
            else
            {
                ...
                dataGridView2.Rows[dgvRows4 + 2].Cells[1].Style.Font = label7.Font;
                dataGridView2.Rows[dgvRows4 + 2].Cells[2].Style.Font = label7.Font;
                CreditDebit = "Credit";
            }          
            ...
            // row1 = new string[] { "", "Balance B/d", cc.ToString("#,###.00") };
            // string[] row = new string[] { "", "Balance C/d", cc.ToString("#,###.00") };
        }
        ...
        public void EPDFReport(DataGridView dgw, string filename)
        {
            ...
            if (savefiledialoge.ShowDialog() == DialogResult.OK)
            {
                using (FileStream stream = new FileStream(savefiledialoge.FileName, FileMode.Create))
                {
                    ...
                    pdfdoc.Open();
                    Paragraph para = new Paragraph(" Cash Book ");
        ...
        {
            EPDFReport(dataGridView3, " Cash Book - " + DateTime.Now.ToString("dd.MM.yyyy"));
            OpenFileDialog dlg = new OpenFileDialog();
            dlg.Filter = "pdf files (*.pdf) |*.pdf;";
            dlg.FileName = (" Cash Book - " + DateTime.Now.ToString("dd.MM.yyyy") + ".pdf");
            dlg.ShowDialog();
        ...
        private void cmdPost_Click(object sender, EventArgs e)
        {
            try
            {
                if (CreditDebit == "Credit")
                {
                   ...
                   + "Cash" + "', '" + "0.00" + "', '" + DD1 + "', '" + "Others" + "', '" + "Current Assets" + "', '" + "Cash" + "')", con);
                    con.Open();
                    cmd.ExecuteNonQuery();
                    con.Close();
                ...
                {
                    OleDbCommand cmd = new OleDbCommand("Insert into TB(Particular, Debit, Credit, PLACType, BalALType, CashFlow) values('"
                    + "Cash" + "', '" + DD1 + "', '" + "0.00" + "', '" + "Others" + "', '" + "Current Assets" + "', '" + "Cash" + "')", con);
                    con.Open();
                    cmd.ExecuteNonQuery();
                    con.Close();
                    MessageBox.Show("Record Posting Sucessfully!", "MESSAGE BOX", MessageBoxButtons.OK, MessageBoxIcon.Information);
