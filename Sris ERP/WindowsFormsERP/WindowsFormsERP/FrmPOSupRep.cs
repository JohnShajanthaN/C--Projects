...
{
    public partial class FrmPOSupRep : Form
    {
        int dgvRows = 0;
        string Lst1 = "";
        double pp1, qq1;
        ...
        private void DGViewSize()
        {
            ...
            dataGridView1.Columns[1].HeaderText = " PONO";
            dataGridView1.Columns[2].HeaderText = " PODate";
            dataGridView1.Columns[3].HeaderText = " St-No.";
            dataGridView1.Columns[4].HeaderText = " Description";
            dataGridView1.Columns[5].HeaderText = " CostPrice";
            dataGridView1.Columns[6].HeaderText = " Weight";
            dataGridView1.Columns[7].HeaderText = " OrderQty";
            dataGridView1.Columns[8].HeaderText = " ItemValue";
            ...
            con.Open();
            OleDbCommand cmd1 = new OleDbCommand("select * from Purchase where Supplier='" + Lst1 + "' AND (PONO Like '%" + TxtSer.Text + "%' OR StockNumber Like '%" + TxtSer.Text + "%' OR Description Like '%" + TxtSer.Text + "%') ", con);
            cmd1.Connection = con;
            ...
            while (rdr.Read())
            {
                ...
                dataGridView1.Rows[dgvRows - 1].Cells[2].Value = Convert.ToDateTime(rdr["PODate"].ToString()).ToString("dd/MM/yyyy");
                dataGridView1.Rows[dgvRows - 1].Cells[3].Value = rdr["StockNumber"].ToString();
                dataGridView1.Rows[dgvRows - 1].Cells[4].Value = rdr["Description"].ToString();
                dataGridView1.Rows[dgvRows - 1].Cells[5].Value = Convert.ToDouble(rdr["CostPrice"].ToString()); 
                dataGridView1.Rows[dgvRows - 1].Cells[6].Value = rdr["Weight"].ToString();
                dataGridView1.Rows[dgvRows - 1].Cells[7].Value = rdr["OrderQty"].ToString() + " " + rdr["Unit"].ToString();
                dataGridView1.Rows[dgvRows - 1].Cells[8].Value = Convert.ToDouble(rdr["ItemValue"].ToString());
        ...
        public void EPDFReport(DataGridView dgw, string filename)
        {
            ...
            float[] widths = new float[] { 60f, 120f, 110f, 95f, 280f, 110f, 95f, 95f, 120f };
            ...
            foreach (DataGridViewRow row in dgw.Rows)//Add Data Row  
            {
                foreach (DataGridViewCell cell in row.Cells)
                {
                    ...
                    else if (cell.ColumnIndex.ToString() == "0" || cell.ColumnIndex.ToString() == "5" || cell.ColumnIndex.ToString() == "8")
                    {
            ...
            if (savefiledialoge.ShowDialog() == DialogResult.OK)
            {
                using (FileStream stream = new FileStream(savefiledialoge.FileName, FileMode.Create))
                {
                    ...
                    Paragraph para0 = new Paragraph("Purchase Order on " + DateTime.Now.ToString("dd.MM.yyyy"));
        ...
        private void FrmPOSupRep_Load(object sender, EventArgs e)
        {
            Lst1 = FrmPOSupList.Lst;
            this.label1.Text = "Purchase Order - " + this.Text;
            Refresh();
            this.Refresh();
            Refresh();
            this.Refresh();
        ...
        private void FrmPOSupRep_FormClosing(object sender, FormClosingEventArgs e)
        {
            FrmPOSupList FPOSupList = new FrmPOSupList();
            FPOSupList.Visible = true;
        }
