...
{
    public partial class StuPerInfoRep : Form
    {
        OleDbConnection con = new OleDbConnection(@"Provider=Microsoft.ACE.OLEDB.12.0;Data Source=|DataDirectory|\Dewmas.mdb");

        public StuPerInfoRep()
        {
            InitializeComponent();
        }

        private void StuPerInfoRep_Load(object sender, EventArgs e)
        {
        ...
        public void EPDFReport(DataGridView dgw, string filename)
        {
            ...
            if (savefiledialoge.ShowDialog() == DialogResult.OK)
            {
                using (FileStream stream = new FileStream(savefiledialoge.FileName, FileMode.Create))
                {
                    ...
                    pdfdoc.Open();
                    Paragraph para = new Paragraph("Student Personal Information Report");
        ...
        private void TxtSer_TextChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                cmd.CommandText = "select * from StuReg where StuID Like '%" + TxtSer.Text + "%' or FName Like '%" + TxtSer.Text + "%' or NIC Like '%" + TxtSer.Text + "%'or StuCat Like '%" + TxtSer.Text + "%' or Gender Like '%" + TxtSer.Text + "%'";
                cmd.ExecuteNonQuery();
        ...
        {
            EPDFReport(dataGridView1, "Student Personal Information Report-" + DateTime.Now.ToString("dd.MM.yyyy"));
            OpenFileDialog dlg = new OpenFileDialog();
            // set file filter of dialog   
            dlg.Filter = "pdf files (*.pdf) |*.pdf;";
            dlg.FileName = ("Student Personal Information Report-" + DateTime.Now.ToString("dd.MM.yyyy") + ".pdf");
            dlg.ShowDialog();
