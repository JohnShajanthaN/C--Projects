...
{
    public partial class FrmJobSekPay : Form
    {
        ...
        private void FrmJobSekPay_Load(object sender, EventArgs e)
        {
        ...
        private void ComList()
        {
            try
            {
                ...
                OleDbCommand cmd = con.CreateCommand();
                cmd.CommandType = CommandType.Text;
                cmd.CommandText = "select JskID from JobSkReg";
                cmd.ExecuteNonQuery();
                DataTable dt = new DataTable();
                OleDbDataAdapter da = new OleDbDataAdapter(cmd);
                da.Fill(dt);
                ...
                foreach (DataRow dr in dt.Rows)
                {
                    comRegID.Items.Add(dr["JskID"].ToString());
                }
                con.Close();
            }
            catch (Exception x)
        ...
        private void cmdAdd_Click(object sender, EventArgs e)
        {
            try
            {
                OleDbCommand cmd = new OleDbCommand("Insert into JobSkPay values('"
        ...
        private void cmdEdit_Click(object sender, EventArgs e)
        {
            try
            {
                OleDbCommand cmd = new OleDbCommand("Update JobSkPay SET JskID='" + comRegID.Text + "', Payment='" + txtPay.Text + "', PayDate='" + dtpPayDate.Text + "' WHERE StuID='" + dataGridView1.SelectedRows[0].Cells[0].Value + "'", con);
                con.Open();
                cmd.ExecuteNonQuery();
                con.Close();
        ...
        private void cmdDel_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    string txt = "DELETE FROM [JobSkPay] Where [JskID]='" + dataGridView1.SelectedRows[0].Cells[0].Value.ToString() + "'";
                    OleDbDataAdapter da = new OleDbDataAdapter(txt, con);
                    DataSet ds = new DataSet();
                    da.Fill(ds);
        ...
        {
            FrmJobSekPayRep JSPR = new FrmJobSekPayRep();
            JSPR.Show();
        }
        ...
        private void dataGridView1_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                string txt = "SELECT * FROM [JobSkPay] Where JskID='" + dataGridView1.SelectedRows[0].Cells[0].Value.ToString() + "'";
                OleDbDataAdapter da = new OleDbDataAdapter(txt, con);
                DataSet ds = new DataSet();
                da.Fill(ds);
                fromDGView();
