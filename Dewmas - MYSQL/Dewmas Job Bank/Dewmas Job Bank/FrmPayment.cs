...
namespace Dewmas_Job_Bank
{
    ...
    public partial class FrmPayment : Form
    {
        ...
        private void cmdAdd_Click(object sender, EventArgs e)
        {
            try
            {
                SqlCommand cmd = new SqlCommand("Insert into StuPay values('"
        ...
        private void cmdEdit_Click(object sender, EventArgs e)
        {
            try
            {
                SqlCommand cmd = new SqlCommand("Update StuPay SET StuID='" + comRegID.Text + "', Payment='" + txtPay.Text + "', PayDate='" + dtpPayDate.Text + "' WHERE StuID='" + dataGridView1.SelectedRows[0].Cells[0].Value + "'", con);
                con.Open();
                cmd.ExecuteNonQuery();
                con.Close();
